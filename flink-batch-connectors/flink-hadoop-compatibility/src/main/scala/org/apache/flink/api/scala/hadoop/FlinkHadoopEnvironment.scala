...
class FlinkHadoopEnvironment(parentEnv: ExecutionEnvironment) {
  ...
  def createHadoopInput[K, V](
    ...
    val hadoopInputFormat = new MapredHadoopInputFormat[K, V](mapredInputFormat, key, value, job)
  ...
  def createHadoopInput[K, V](
    ...
    val hadoopInputFormat =
      new MapreduceHadoopInputFormat[K, V](mapreduceInputFormat, key, value, job)
    getParentEnv.createInput(hadoopInputFormat)
  }
  /**
